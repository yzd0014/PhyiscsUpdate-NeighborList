	...
	namespace Physics {
		bool  CollisionDetection(sRigidBodyState  & i_object_A, sRigidBodyState & i_object_B, float i_dt, Math::sVector &o_normal4A, float &o_collisionTime) {
			...
			if (maxCloseTime <= minOpenTime) {
				...
				//UserOutput::DebugPrint("number of axis: %d", axisToCheck.size());
				return true;
			}
		...
		bool FindEarliestCollision(std::vector<GameCommon::GameObject *> & i_allGameObjects, float i_dt, CollisionPair & o_earliestCollision) {
			...
			NeighborList neighborList(i_allGameObjects);
...
#pragma omp for
				for (int m_c = 0; m_c < neighborList.numCellsXYZ; m_c++) {
					if (neighborList.head[m_c] == EMPTY) continue;

					int i_y = m_c / neighborList.numCellsXZ;
					int i_z = (m_c % neighborList.numCellsXZ) / neighborList.numCells[x_ii];
					int i_x = (m_c % neighborList.numCellsXZ) % neighborList.numCells[x_ii];

					for (int n_y = i_y - 1; n_y <= i_y + 1; n_y++) {
						if (n_y < 0 || n_y > neighborList.numCells[y_ii] - 1) continue;
						for (int n_z = i_y - 1; n_z <= i_z + 1; n_z++) {
							if (n_z < 0 || n_z > neighborList.numCells[z_ii] - 1) continue;
							for (int n_x = i_x - 1; n_x < i_x + 1; n_x++) {
								if (n_x < 0 || n_x > neighborList.numCells[x_ii] - 1) continue;

								int n_c = neighborList.numCellsXZ * n_y + neighborList.numCells[x_ii] * n_z + n_x;

								if (neighborList.head[n_c] == EMPTY) continue;

								int i = neighborList.head[m_c];
								while (i != EMPTY) {
									int j = neighborList.head[n_c];
									while (j != EMPTY) {
										if (i < j) {
											Math::sVector colNormal4A;
											float colTime;
...
#pragma omp critical  				
										...
										j = neighborList.linkedCellist[j];
									}
									i = neighborList.linkedCellist[i];
								}
